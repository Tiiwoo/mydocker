...
// 初始化一个 IPAM 的对象，默认使用 /var/run/mydocker/network/ipam/subnet.json 作为分配信息存储位置
...
func (ipam *IPAM) Allocate(subnet *net.IPNet) (ip net.IP, err error) {
	...
	if _, exist := (*ipam.Subnets)[subnet.String()]; !exist {
		...
		// 2^(size - one) 等价于 1 << uint8(size - one)
		// 左移一位就是扩大两倍
	...
	for c := range (*ipam.Subnets)[subnet.String()] {
		...
		if (*ipam.Subnets)[subnet.String()][c] == '0' {
			...
			// Go 的字符串，创建之后就不能修改所以通过转换成 byte 数组，修改后再转换成字符串赋值
			...
			ip = subnet.IP
			/*
				还需要通过网段的 IP 与上面的偏移相加计算出分配的 IP 地址，由于 IP 地址是 uint 的一个数组，
			...
			*/
			...
			// 由于此处 IP 是从 1 开始分配的（0 被网关占了），所以最后再加 1，最终得到分配的 IP 172.17.0.20
			ip[3] += 1
			break
		}
	}
