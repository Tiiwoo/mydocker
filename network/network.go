...
func Init() error {
	...
	err := filepath.Walk(defaultNetworkPath, func(nwPath string, info os.FileInfo, err error) error {
		...
		if e := nw.load(nwPath); e != nil {
			logrus.Errorf("error load network: %s", e)
		}
...
func configEndpointIpAddressAndRoute(ep *Endpoint, info *container.Info) error {
	...
	// net namespace 中默认本地地址 127 的，网卡是关闭状态的
	// 启动它以保证容器访问自己的请求
	...
	// 相当于 route add -net 0.0.0.0/0 gw (Bridge网桥地址) dev (容器内的 Veth 端点设备)
